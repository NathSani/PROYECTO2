...
def ciudad_plantas():
    for keys in plantas:
        if ciudad in keys:
            for keys in consumo_energia['Coca Codo Sinclair']:
                if keys == ciudad:
                    total_cocacodo = (sum(consumo_energia['Coca Codo Sinclair'][(ciudad)]['consumos']))
                    print('Coca Codo Sinclair: ',total_cocacodo, 'MWh')
                    continue
            for keys in consumo_energia['Sopladora']:
                if keys  == ciudad:
                    total_sopladora = (sum(consumo_energia['Sopladora'][ciudad]['consumos']))        
                    print('Sopladora: ',total_sopladora, 'MWh')
        if ciudad not in plantas:
            print ('La ciudad ingresada no cuenta con registros disponibles')
            break

def rec ():
    ciudades_region = inf[region]
    total_consumo = 0
...
while op != 0:
    ...
    while op == 3:
        region = input('Region: ')
        if region not in inf.keys():
            print ('¡La regiónn no existe!')
            print('¡Regresando al menú principal!')
            break
        if region in inf.keys():
            rec()
            print('¡Regresando al menú principal')
            break      
``` 
